...
@Configuration
@EnableSwagger2
@ComponentScan(basePackages = ["cn.lannie"])
class SwaggerConfig : WebMvcConfigurerAdapter() {

    /*添加自定义异常信息*/
    private val responseMessages = object : ArrayList<ResponseMessage>() {
        private val serialVersionUID = 1L

        init {
            add(ResponseMessageBuilder().code(200).message("请求成功").build())
            add(ResponseMessageBuilder().code(400).message("请求参数错误").responseModel(ModelRef("Error")).build())
            add(ResponseMessageBuilder().code(401).message("权限认证失败").responseModel(ModelRef("Error")).build())
            add(ResponseMessageBuilder().code(404).message("请求资源不存在").responseModel(ModelRef("Error")).build())
            add(ResponseMessageBuilder().code(405).message("请求方式不支持").responseModel(ModelRef("Error")).build())
            add(ResponseMessageBuilder().code(500).message("服务器内部错误").responseModel(ModelRef("Error")).build())
        }
    }
    ...
    private fun apiInfo(): ApiInfo {
        val contact = Contact("付为地", "", "1335157415@qq.com")
